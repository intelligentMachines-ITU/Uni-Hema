...
class AlbumentationsClassification:
        def __init__(self):
            import albumentations as A
            self.transform = A.Compose([
                A.Blur(p=0.01),
        ...
        def __call__(self, img, target=None, p=1.0):
            """
            Args:
                img (PIL.Image): Input image.
                target (int, optional): Class label (can be passed through unchanged).
                p (float): Probability of applying the transform.
            
            Returns:
                img (PIL.Image): Augmented image.
                target (int): Same as input target.
            """
            img_np = np.array(img)
            if self.transform and random.random() < p:
