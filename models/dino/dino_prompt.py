...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        self.global_pool_proj = nn.Conv2d(2048, 256, kernel_size=1)
        ...
        self.T5_tokenizer  = T5Tokenizer.from_pretrained("/home/iml/DINO/coco_data/coco_data/t5_models/mask_finetuned")
        self.T5_model = T5ForConditionalGeneration.from_pretrained("/home/iml/DINO/coco_data/coco_data/t5_models/mask_finetuned").cuda()
        ...
        self.global_pos = nn.Parameter(torch.randn(1, hidden_dim, 1, 1))
        self.global_src = nn.Parameter(torch.randn(1, hidden_dim, 1, 1))
...
# ]
        ...
        return encoder_hidden_states, encodings.attention_mask, decoder_input_ids,labels
    def decode_from_encoder(self, encoder_hidden_states, attention_mask, decoder_input_ids, labels,model, 
        ...
        generated_ids = model.generate(
            input_ids=None,  # we provide encoder_hidden_states directly
    ...
    def forward(self, samples: NestedTensor,  prompt, targets:List=None,):
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs or l == self.num_feature_levels - 1: 
                    # take last backbone feature
                    feat_last = features[-1].tensors
                    if l == self.num_feature_levels - 1:  
                        # ✅ pool BEFORE projection for the very last level
                        # feat_last = F.adaptive_avg_pool2d(feat_last, (1, 1))
                        # src = self.global_pool_proj(feat_last) 

                        #### learnable 
                        Bts = feat_last.size(0)

                        # Use learnable token instead of pooled feature
                        src = self.global_src.expand(Bts, -1, -1, -1) 
                        
                        # ----- special handling for mask & pos -----
                        # mask becomes 1x1 (all False, i.e., no padding at pooled scale)
                        mask = torch.zeros((feat_last.size(0), 1, 1), dtype=torch.bool, device=feat_last.device)

                        # positional encoding: just use a learnable embedding (not sine PE)
                        pos_l = self.global_pos.expand_as(src) # or register a nn.Parameter if you want it learnable
                    else:
                        src = self.input_proj[l](feat_last)
                        m = samples.mask
                        mask = F.interpolate(m[None].float(), size=src.shape[-2:]).to(torch.bool)[0]
                        pos_l = self.backbone[1](NestedTensor(src, mask)).to(src.dtype)

                else:
                    # interpolate from previous projected level
        ...
        text_embeddings, attention_mask, decoder_input_ids,labels_encoder = self.encode_prompts(prompt,completed_texts, self.T5_model, self.T5_tokenizer,)
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal,predictions_class, predictions_mask,mask_features,enc_class_tokens_encder,encoder_output,decoder_output = self.transformer(srcs,text_embeddings,prompt, masks, input_query_bbox, poss,input_query_label,attn_mask)
