...
#     def __init__(self, enc_channels=256, bottleneck_channels=256,
#                  decoder_channels=1, out_channels=1, output_size=(512, 512)):
#         super().__init__()
#         self.output_size = output_size

#         fused_channels = enc_channels + bottleneck_channels

#         # -----------------------------
#         # Fusion conv block
#         # -----------------------------
#         self.fusion_conv = nn.Sequential(
#             nn.Conv2d(fused_channels, 256, kernel_size=3, padding=1),
#             nn.BatchNorm2d(256),
...
#         # -----------------------------
#         # 1st PixelShuffle Upsample (×2)
#         # -----------------------------
#         self.up1 = nn.Sequential(
#             nn.Conv2d(256 + decoder_channels, 512, kernel_size=3, padding=1),
#             nn.PixelShuffle(2),  # (512 → 128 channels, H×2, W×2)
#             nn.ReLU(inplace=True)
#         )

#         # -----------------------------
#         # 2nd PixelShuffle Upsample (×2)
#         # -----------------------------
#         self.up2 = nn.Sequential(
#             nn.Conv2d(128, 256, kernel_size=3, padding=1),
#             nn.PixelShuffle(2),  # (256 → 64 channels, H×2, W×2)
#             nn.ReLU(inplace=True)
#         )
#         self.up3 = nn.Sequential(
#             nn.Conv2d(out_channels, 64, kernel_size=3, padding=1),
#             nn.PixelShuffle(2),   # 64 → 16 channels, Hx2, Wx2
#             nn.ReLU(inplace=True)
#         )

#         # -----------------------------
#         # Final 3×3 output conv
#         # -----------------------------
#         self.out_conv = nn.Conv2d(64, out_channels, kernel_size=3, padding=1)

#         # -----------------------------
#         # Corner refinement block
#         # -----------------------------
...
#     def forward(self, x, enc_features, b_features):
#         """
#         x: decoder logits [B, H, W]
#         enc_features: encoder features [B, Cenc, H, W]
#         b_features: bottleneck features [B, Cb, H, W]
#         """
#         # Fuse encoder + bottleneck
#         fused = torch.cat([enc_features, b_features], dim=1)   # [B, C_enc+C_bottleneck, H, W]
#         fused = self.fusion_conv(fused)                         # [B, 256, H, W]

#         # Add decoder logits as channel
#         x = x.unsqueeze(1)                                      # [B, 1, H, W]
#         x = torch.cat([fused, x], dim=1)                        # [B, 257, H, W]

#         # PixelShuffle Upsample stages
#         out = self.up1(x)                                       # [B, 128, H*2, W*2]
#         out = self.up2(out)                                     # [B, 64,  H*4, W*4]

#         # Output conv
#         out = self.out_conv(out)                                # [B, outC, H*4, W*4]

#         # Corner refinement
#         out = self.refine(out)

#         # Force exact final size
...
class LearnableUpsample(nn.Module):
    def __init__(self, enc_channels=256, bottleneck_channels=256, decoder_channels=1, out_channels=1, output_size=(512, 512)):
        super().__init__()
        self.output_size = output_size
        ...
        self.upsample = nn.Sequential(
            ...
            nn.Conv2d(256 + decoder_channels, 128, kernel_size=3, padding=1),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            ...
            # do NOT include Sigmoid if using BCEWithLogitsLoss
        )

    def forward(self, x, enc_features, b_features):
        """
        x: decoder logits [B, H, W]
        enc_features: encoder features [B, C, H, W]
        b_features: bottleneck features [B, C, H, W]
        """
        # Fuse encoder + bottleneck
        fused = torch.cat([enc_features, b_features], dim=1)  # [B, C_enc + C_bottleneck, H, W]
        fused = self.fusion_conv(fused)                        # [B, 256, H, W]

        # Add decoder logits as extra channel
        x = x.unsqueeze(1)                                     # [B, 1, H, W]
        x = torch.cat([fused, x], dim=1)                       # [B, 257, H, W]

        # Upsample
        out = self.upsample(x)    
        out=out# [B, out_channels, H*4, W*4]
...
#     def __init__(self, d_model_img=256, d_model_txt=768, d_model_q=256, n_heads=8, num_layers=2):
#         super().__init__()

#         # Projections
#         self.txt_proj = nn.Linear(d_model_txt, d_model_q)   # text -> query space
#         self.img_proj = nn.Linear(d_model_img, d_model_q)   # image encoder feats -> query space
#         self.img_dec_proj = nn.Linear(d_model_img, d_model_q)  # image decoder feats -> query space

#         # Stacked cross-attn blocks
#         self.layers = nn.ModuleList([
#             nn.ModuleDict({
#                 "cross_attn_img": nn.MultiheadAttention(d_model_q, n_heads, batch_first=True),
#                 "cross_attn_imgdec": nn.MultiheadAttention(d_model_q, n_heads, batch_first=True),
#                 "ffn": nn.Sequential(
#                     nn.Linear(d_model_q, d_model_q * 4),
#                     nn.GELU(),
#                     nn.Linear(d_model_q * 4, d_model_q),
#                 ),
#                 "norm1": nn.LayerNorm(d_model_q),
#                 "norm2": nn.LayerNorm(d_model_q),
#                 "norm3": nn.LayerNorm(d_model_q),
#             })
#             for _ in range(num_layers)
#         ])

#         # Project back to text hidden space
#         self.out_proj = nn.Linear(d_model_q, d_model_txt)

#     def forward(self, img_feats_enc, img_feats_dec,txt_feats):
#         """
#         txt_feats:    (B, 64, 768)     -> text embeddings (used as queries)
#         img_feats_enc:(B, 2560, 256)   -> image encoder features
#         img_feats_dec:(B, 600, 256)    -> image decoder features

#         Returns:
#             q_out: (B, 64, 768)
#         """
#         # Project into query space
#         q = self.txt_proj(txt_feats)               # (B, 64, d_model_q)
#         img_feats_enc_proj = self.img_proj(img_feats_enc)     # (B, 2560, d_model_q)
#         img_feats_dec_proj = self.img_dec_proj(img_feats_dec) # (B, 600, d_model_q)

#         # Stacked layers
#         for layer in self.layers:
#             # --- Cross-attn over image encoder ---
#             q_res = q
#             q, _ = layer["cross_attn_img"](q, img_feats_enc_proj, img_feats_enc_proj)
#             # q = layer["norm1"](q + q_res)

#             # --- Cross-attn over image decoder ---
#             # q_res = q
#             # q, _ = layer["cross_attn_imgdec"](q, img_feats_dec_proj, img_feats_dec_proj)
#             # q = layer["norm2"](q + q_res)

#             # --- Feed-forward ---
#             # q_res = q
#             q = layer["ffn"](q)
#             # q = layer["norm3"](q + q_res)

#         # Final projection back to text hidden size
#         q_out = self.out_proj(q)  # (B, 64, 768)
#         return q_out
class MultimodalQFormerWithTextCrossAttn(nn.Module):
    def __init__(self, num_queries=64, d_model_img=256, d_model_txt=768, d_model_q=256, n_heads=8, num_layers=2):
        super().__init__()
        # Learnable queries
        self.queries = nn.Parameter(torch.randn(num_queries, d_model_q))

        # Projections
        self.img_proj = nn.Linear(d_model_img, d_model_q)  # image feats -> query dim
        self.txt_proj = nn.Linear(d_model_txt, d_model_q)  # text feats -> query dim

        # Stack of cross-attention blocks
        self.layers = nn.ModuleList([
            nn.ModuleDict({
                "cross_attn_img": nn.MultiheadAttention(d_model_q, n_heads, batch_first=True),
                "cross_attn_txt": nn.MultiheadAttention(d_model_q, n_heads, batch_first=True),
                "ffn": nn.Sequential(
                    nn.Linear(d_model_q, d_model_q * 4),
                    nn.GELU(),
                    nn.Linear(d_model_q * 4, d_model_q),
        ...
        # Final projection to T5 hidden size
        self.out_proj = nn.Linear(d_model_q, d_model_txt)

    def forward(self, img_feats,decoder_output, txt_feats=None):
        """
        img_feats: (B, 900, d_model_img)
        txt_feats: (B, L, d_model_txt)  # optional, e.g., from T5 encoder
        """
        B = img_feats.size(0)
        q = self.queries.unsqueeze(0).expand(B, -1, -1)  # (B, num_queries, d_model_q)

        # Project features
        img_feats_proj = self.img_proj(img_feats)  # (B, 900, d_model_q)
        txt_feats_proj = self.txt_proj(txt_feats) if txt_feats is not None else None

        # Pass through stacked cross-attn blocks
        for layer in self.layers:
            # Cross-attn over image
            q_res = q
            q, _ = layer["cross_attn_img"](q, img_feats_proj, img_feats_proj)
            q = layer["norm1"](q + q_res)

            # Cross-attn over text (optional)
            if txt_feats_proj is not None:
                q_res = q
                q, _ = layer["cross_attn_txt"](q, txt_feats_proj, txt_feats_proj)
                q = layer["norm2"](q + q_res)
        ...
        # Project back to T5 dice
        q_out = self.out_proj(q) 
        q_out= q_out+txt_feats # (B, num_queries, d_model_txt)
        return q_out
# class MultimodalQFormerWithTextCrossAttn(nn.Module):
...
#         # vision_feats
#         vision_feats = encoder_output.detach()#torch.cat([encoder_output, decoder_output], dim=1)  # [B, 2054, vision_dim]
...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        self.xformer = MultimodalQFormerWithTextCrossAttn()
        ...
        self.upsampler = LearnableUpsample()
        ...
        self.T5_model = T5ForConditionalGeneration.from_pretrained("google/flan-t5-base").cuda()
        # self.T5_tokenizer  = T5Tokenizer.from_pretrained("/media/iml_abdul/40a71b91-5aab-40ee-a60a-16e92f17d93f/home/iml/DINO/coco_data/coco_data/t5_models/mm_leukemia_qa_wbcatt_26_10_2nd_traing_step_2")
        # self.T5_model = T5ForConditionalGeneration.from_pretrained("/media/iml_abdul/40a71b91-5aab-40ee-a60a-16e92f17d93f/home/iml/DINO/coco_data/coco_data/t5_models/mm_leukemia_qa_wbcatt_26_10_2nd_traing_step_2").cuda()
    ...
    def encode_prompts(self, input_text,target_text, T5_model, T5_tokenizer, max_length=64, device='cuda'):
        ...
        if target_text:
            ...
            processed_input = [
                            f"This image is for the detection of {text if isinstance(text, str) else ', '.join(text)} cells"
                            for text in input_text
]
#         processed_text = [
...
# ]
        ...
        return encoder_hidden_states, encodings.attention_mask, decoder_input_ids,labels,encodings.input_ids
    def decode_from_encoder(self, encoder_hidden_states, attention_mask, decoder_input_ids,encoder_input_ids, labels,model, 
            tokenizer, max_length=64, device='cuda'):
        
        
        
        """
        Decode from encoder outputs (teacher forcing if labels provided)
        """
        ...
        generated_ids = model.generate(
            input_ids=encoder_input_ids.to(device),  # we provide encoder_hidden_states directly
    ...
    def forward(self, samples: NestedTensor,  prompt, targets:List=None,):
        ...
        text_embeddings, attention_mask, decoder_input_ids,labels_encoder,encoder_input_ids = self.encode_prompts(prompt,completed_texts, self.T5_model, self.T5_tokenizer,)
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal,predictions_class, predictions_mask,mask_features,enc_class_tokens_encder,encoder_output,decoder_output,_,cls_feature, pooled_feature,backbone_feat,enc_featuers,b_featuers= self.transformer(srcs,text_embeddings,prompt, masks, input_query_bbox, poss,input_query_label,attn_mask)
        ...
        text_embeddings_attented = self.xformer (backbone_feat, decoder_output, text_embeddings)
        ...
        predictions_mask= self.upsampler(predictions_mask[-1][:, 0],enc_featuers,b_featuers)
        # predictions_mask = F.interpolate(predictions_mask[-1][:, 0].unsqueeze(1), size=(512, 512), mode='bilinear', align_corners=False)
        logits_image_featuers=None
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord_list[-1], 'pred_morphology': outputs_morphology[-1], 'pred_mask':predictions_mask,'pred_image_class':logits_image,'pred_image_feat':logits_image_featuers,'loss_text':text_loss,'pred_text':decoded_text,'completed_text':decoded_labels,'encoder_class_feat':enc_class_tokens_encder,'text_embeddings':text_embeddings}
        # if self.aux_loss:
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets,args, return_indices=False):
        ...
        if any(t.get("segmentation", torch.tensor(0)).item() == 1 for t in targets):
            ...
            dice = dice_loss2(pred_probs, gt_masks) # + F.binary_cross_entropy(pred_masks, gt_masks)
            focal = focal_loss(pred_probs, gt_masks)
            # bce = F.binary_cross_entropy(pred_probs, gt_masks)
...
def build_dino(args):
    ...
    if args.use_dn:
        ...
        weight_dict['text_loss'] = args.text_loss_coef
