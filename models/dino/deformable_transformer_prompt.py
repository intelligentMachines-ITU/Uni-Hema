...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs,text_embeddings,prompt, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        memory, enc_intermediate_output, enc_intermediate_refpoints,enc_featuers,b_featuers,class_token= self.encoder(srcs,
                src_flatten, 
        ...
        #########################################################
        lvl_pos_embed_flatten= lvl_pos_embed_flatten[:, :-1, :] 
...
# )
        ...
        cls_token = memory[:, :1, :]
        return hs, references, hs_enc, ref_enc, init_box_proposal, predictions_class, predictions_mask, mask_features,class_token,memory[:, 1:, :],hs[0]
        # hs: (n_dec, bs, nq, d_model)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, 
        ...
        if ref_token_index is not None:
            ...
            intermediate_output = intermediate_ref = None                    # [bs, d_model]
        
        # project for mask features
        global_feature = output[:, -1:, :]          # extract last query
        output = output[:, :-1, :]                # remove last query from output

        # Remove the last spatial shape
        spatial_shapes = spatial_shapes[:-1]
        ...
        return output, intermediate_output, intermediate_ref, mask_features, out_featuers, global_feature
