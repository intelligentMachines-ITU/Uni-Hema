...
# decoded_labels = tokenizer.batch_decode(labels, skip_special_tokens=True)
    ...
    def forward(self, samples: NestedTensor,  prompt, targets:List=None,):
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal,predictions_class, predictions_mask,mask_features,encoder_output,decoder_output,_,class_tokens= self.transformer(srcs,text_embeddings,prompt, masks, input_query_bbox, poss,input_query_label,attn_mask)
        # In case num object=0
        

        logits_image = self.classifier_image(class_tokens.squeeze(1))
        # logits_image, enc_class_tokens = self.classification_decoder(cls_feature, pooled_feature)
        
        logits_image_featuers= self.classifier_feat (class_tokens.squeeze(1))

        hs[0] += self.label_enc.weight[0,0]*0.0
        text_embeddings_attented = self.dformer(class_tokens, decoder_output, text_embeddings)
