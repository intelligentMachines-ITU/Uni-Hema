...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        self.queries = nn.Parameter(torch.randn(1, 1, d_model))
        self.cross_attn = nn.MultiheadAttention(embed_dim=256, num_heads=8, batch_first=True).cuda()
    ...
    def forward(self, srcs,text_embeddings,prompt, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        memory, enc_intermediate_output, enc_intermediate_refpoints,enc_featuers,b_featuers= self.encoder(srcs,
                src_flatten, 
...
# cls_pos = self.cls_pos.expand(bs, -1, -1) 
        ...
        query_batch = self.queries.expand(bs, -1, -1) # (B, num_queries, d_model)

        # Cross-attention: queries attend to memory
        class_tokens, _ = self.cross_attn(query=query_batch, key=memory, value=memory)
        pooled_feature = memory.max(dim=1).values  # (B, d_model)
        class_tokens = torch.cat([class_tokens.squeeze(1), pooled_feature], dim=1)
        class_tokens=self.combined_proj(class_tokens)
        # pooled_features = class_tokens.reshape(class_tokens.size(0), -1)
        # Mean pooling over the sequence dimension (assuming dim=1 is the sequence length)
        # cls_mean = memory.mean(dim=1, keepdim=True)

        # # Concatenate mean features with the original memory along the feature dimension (dim=1)
        # all_features = torch.cat([cls_mean, memory], dim=1)

        # # Cross-attention using the combined features
        # class_tokens,_ = self.cross_attnn(query=cls_mean, key=all_features, value=all_features)
        #########################################################
        # End Encoder
        # - memory: bs, \sum{hw}, c
        ...
        # attended_tgt, _ = self.cross_attnn(query=tgt, key=projected_text_int, value=projected_text_int)

        # Optional: combine original tgt with attended version
        
        # gate = torch.sigmoid(self.gate_layer(torch.cat([tgt, attended_tgt], dim=-1)))
        # alpha = 0.8
        # combined_tgt = (alpha * gate) * attended_tgt + ((1 - alpha * gate)) * tgt
...
# )
        hs, references = self.decoder(
                tgt=tgt.transpose(0, 1), #chnage with prompt
        ...
        cls_token = memory[:, :1, :]
        return hs, references, hs_enc, ref_enc, init_box_proposal, predictions_class, predictions_mask, mask_features,memory[:, 1:, :],hs[0],text_embeddings,class_tokens
        # hs: (n_dec, bs, nq, d_model)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, 
        ...
        return output, intermediate_output, intermediate_ref, mask_features, out_featuers,
